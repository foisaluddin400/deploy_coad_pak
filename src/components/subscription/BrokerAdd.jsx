...
import { useLazySingleGetCouponQuery, usePostCheckoutMutation } from "@/redux/Api/subscriptionApi";
...
const BrokerAdd = ({ openAddModal, setOpenAddModal, subscriptionId }) => {
  ...
  const [triggerCouponCheck, { data: singleCouponData, isFetching }] = useLazySingleGetCouponQuery();
  ...
  useEffect(() => {
    ...
    if (Array.isArray(subscriptionId.price) && subscriptionId?.price?.length > 0) {
      initialPriceObj = subscriptionId?.price[0]; 
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (res?.success) {
        // For Next.js redirect, use window.location.href if external link
